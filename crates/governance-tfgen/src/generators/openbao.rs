...
pub fn generate_project_policies(data: &GovernanceData) -> TfJsonFile {
    ...
    for team in &data.teams {
        if team.team.projects.is_empty() {
            // devops manages its own vault access by hand in openbao.tf
            if team.team.group.slug == "devops" {
                continue;
            }
            add_project_policies(&mut tf, &team.team.group.slug);
        } else {
            for project in &team.team.projects {
                add_project_policies(&mut tf, &project.group.slug);
            }
        }
    }
...
fn add_project_policies(tf: &mut TfJsonFile, slug: &str) {
